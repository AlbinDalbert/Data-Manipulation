...
pub fn caesar_run(){
    ...
    // ask for key
    let key = input("Enter key: ").parse().unwrap();
    // ask for mode
    ...
    println!("{}", match mode.to_lowercase().as_str() {
        "1" => caesar_encrypt(&text, key),
        "2" => caesar_decrypt(&text, key),
        _ => {
...
fn caesar_encrypt(text: &str, key: usize) -> String {
    ...
    for c in text.chars() {
        ...
        if c.is_ascii_lowercase() {
            ...
            new_c = ALPHABET_LOWER.chars().nth((index + key) % 26).unwrap();
        }
        else if c.is_ascii_uppercase() {
            // uppercase
            let index = ALPHABET_UPPER.find(c).unwrap();
            new_c = ALPHABET_UPPER.chars().nth((index + key) % 26).unwrap();
        }
        result.push(new_c);
    }
    result
...
fn caesar_decrypt(text: &str, key: usize) -> String {
    let mut result = String::new();
    for c in text.chars() {
        let mut new_c = c;
        if c.is_ascii_lowercase() {
